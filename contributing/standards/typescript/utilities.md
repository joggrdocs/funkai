...
| Function       | Description               | Example                      |
| -------------- | ------------------------- | ---------------------------- |
| `groupBy`      | Group by key/function     | `groupBy(tools, 'category')` |
| `keyBy`        | Create lookup by key      | `keyBy(tools, 'name')`       |
| `chunk`        | Split into chunks         | `chunk(items, 10)`           |
| `uniq`         | Remove duplicates         | `uniq(array)`                |
| `uniqBy`       | Remove duplicates by key  | `uniqBy(tools, 'name')`      |
...
| `attempt`  | Wrap in try/catch    | `attempt(fn)`        |
...
- [Errors](./errors.md) -- Using attempt() for error handling
