...
| -------------- | -------------------------- | ------------------------------------------- |
| Test file      | `*.test.ts`                | `agent.test.ts`                             |
| Describe block | Feature/function name      | `describe('createAgent', ...)`              |
| Test case      | Should + expected behavior | `it('should return configured agent', ...)` |
...
Use `vi.mock` for module-level mocks and `vi.fn` for individual functions. Replace real I/O (file system, network, AI API calls) with deterministic mocks.
