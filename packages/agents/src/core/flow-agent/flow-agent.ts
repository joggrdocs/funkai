...
export function flowAgent<TInput, TOutput>(
    ...
    try {
      ...
      await fireHooks(
        ...
        wrapHook(overrides && overrides.onFinish, {
          input,
          result: result as import('@/core/agent/types.js').GenerateResult,
          duration,
        })
      )
  ...
  async function stream(
    input: TInput,
    overrides?: FlowAgentOverrides
  ): Promise<Result<import('@/core/agent/types.js').StreamResult<TOutput>>> {
    const inputParsed = config.input.safeParse(input)
    if (!inputParsed.success) {
      return {
        ok: false,
    ...
    const streamResult: import('@/core/agent/types.js').StreamResult<TOutput> = {
